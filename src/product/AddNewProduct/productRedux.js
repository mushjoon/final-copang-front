...
const setItemComment = createAction(
  SET_ITEM_COMMENT,
  (itemComment) => itemComment
);
const addFormList = createAction(ADD_FORM_LIST, (form) => form);
const delFormList = createAction(DEL_FORM_LIST, (idx) => idx);
const modFormList = createAction(MOD_FORM_LIST, (idx, form) => ({
  form: form,
  idx: idx,
}));
const modFormOne = createAction(MOD_FORM_ONE, (idx, key, value) => ({
  idx: idx,
  key: key,
  value: value,
}));
...
export const setter = handleActions(
  {
    ...
    [DEL_FORM_LIST]: (state, action) => ({
      ...state,
      itemDetailFormList: state.itemDetailFormList.filter(
  ...
  return (
    <div>
      <Example
        ...
        delFormList={delFormList}
        modFormList={modFormList}
        modFormOne={modFormOne}
      />
    </div>
  );
};
